...
### Join for workstation ID and primary department ID, department name & site details

workstation_drop <- glue("DROP TABLE PROD_WORKSTATION_MAPPING")
workstation_query <- glue("CREATE TABLE PROD_WORKSTATION_MAPPING AS
...
### only filtering for workstations that are mapped in Epic
...
user_activity_query <- glue("CREATE TABLE USER_ACTIVITY_HOUR_TABLE AS
                            ...
                            FROM
                            (SELECT a.* FROM 
                            ((SELECT * FROM UAL_ACTIVITY_HOURS WHERE TO_CHAR(ACTIVITY_HOUR_DTTM,'YYYY-MM-DD') = '2024-03-04') a
